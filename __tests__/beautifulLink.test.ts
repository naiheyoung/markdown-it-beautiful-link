...
import beautifulLink from '../dist/index.mjs'
import type { CreateSpan, CreateSimpleToken } from '../dist/index.mjs'
import { it, expect } from 'vitest'

it('beautifulLink', () => {
  const md = new MarkdownIt()
  md.use(beautifulLink, {
    capturers: (createSpan: CreateSpan, createSimpleToken: CreateSimpleToken) => {
      return {
        'google.com': (link: string) => {
          if (!link) return []
...
[BeautifulLink](https://github.com/naiheyoung/markdown-it-beautiful-link)
[](https://google.com)
`)
